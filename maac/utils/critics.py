...
class AttentionCritic(nn.Module):
    ...
    def __init__(self,
        ...
        for sdim, adim in self.sa_sizes:
            ...
            if norm_in:
                state_encoder.add_module('s_enc_bn',
                                         nn.BatchNorm1d(sdim, affine=False))
        ...
        for i in range(attend_heads):
            ...
            self.value_extractors.append(
                nn.Sequential(nn.Linear(hidden_dim, attend_diㄑm),
                              nn.LeakyReLU()))
    ...
    def forward(self,
        ...
        sa_encodings = [
            encoder(inp) for encoder, inp in zip(self.critic_encoders, inps)
        ]
        ...
        for curr_head_keys, curr_head_values, curr_head_selectors in zip(
            ...
            # iterate over agents
            for i, a_i, selector in zip(range(len(agents)), agents,
                                        curr_head_selectors):
                keys = [k for j, k in enumerate(curr_head_keys) if j != a_i]
                values = [
                    v for j, v in enumerate(curr_head_values) if j != a_i
                ]
                ...
                scaled_attend_logits = attend_logits / np.sqrt(
                    keys[0].shape[1])
        ...
        for i, a_i in enumerate(agents):
            ...
            agent_rets = []
            critic_in = torch.cat((s_encodings[i], *other_all_values[i]),
                                  dim=1)
            ...
            if logger is not None:
                logger.add_scalars(
                    'agent%i/attention' % a_i,
                    dict(('head%i_entropy' % h_i, ent)
                         for h_i, ent in enumerate(head_entropies)), niter)
