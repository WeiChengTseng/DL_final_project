...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--Ga", type=float, default=0.97, help="Discount coefficient")
    ...
    parser.add_argument("--UpdateParam",type=int,default=1,
    ...
    #initize!!
    Env = SocTwoEnv(ENVPATH, worker_id=0, train_mode=True,render=False)
    time_step_0 = 0
    time_step_1 = 0
    time_step_2 = 0
    time_step_3 = 0
    ...
    while episode < opt.MaxEp:
                    ...
                    while True:
                        ...
                        if (len(np.argwhere(cur_done[0]).flatten()) + len(np.argwhere(cur_done[1]).flatten()) == 32):
                            ...
                            if team =="red":
                                ...
                                print("team_{} test result {} time on episode {}".format(team,iter_test,episode),file=f_h)
                                ...
                                total_w_number += w_number
                                total_l_number += l_number
                            else:
                                print("team_{} test result {} time on episode {}".format(team,iter_test,episode))
                                print("team_{} test result {} time on episode {}".format(team,iter_test,episode),file=f_h)
                            ...
                            if iter_test == test_loop:
                                ...
                                print("total win fields: ", total_w_number,file=f_h)
                                print("total lose fields: ", total_l_number,file=f_h)
                                print("total win fields: ", total_w_number)
                                print("total lose fields: ", total_l_number)
                                print("draw fields number: ", test_loop*8 - (total_w_number+total_l_number),file=f_h)
                                print("draw fields number: ", test_loop*8 - (total_w_number+total_l_number))
        ...
        while True:
            ...
            action_striker_distr, action_goalie_distr = Maddpg_object.select_action_train(
                pre_sta_s, pre_sta_g)
            # print("action_striker_action: ",action_striker_distr[[0,2,4,6,8]])
            # striker_distr=action_striker_distr
            # goalie_distr = action_goalie_distr
            # striker_distr=F.gumbel_softmax(striker_distr, dim=-1, tau=10)
            # goalie_distr[:,:5]=F.gumbel_softmax(goalie_distr[:,:5], dim=-1, tau=10)
            # print("action_striker_action: ",action_striker_distr[[0,2,4,6,8]])
            
            # action_striker = Categorical(logits=action_striker_distr).sample()
            # action_goalie = Categorical(logits=action_goalie_distr[:,:5]).sample()
            if time_step % 1000 == 0:
                print('action_striker_distr: ',action_striker_distr)
                print('action_goalie_distr: ',action_goalie_distr)
            action_striker = torch.argmax(action_striker_distr,1).numpy()
            action_goalie  =  torch.argmax(action_goalie_distr,1).numpy()
            ...
            Buffer.update_transition(pre_sta_s, pre_sta_g, action_striker,
                                     action_goalie, action_striker_distr,action_goalie_distr,
                                      reward_s, reward_g,
