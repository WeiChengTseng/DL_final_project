...
from maac_double.utils.buffer import ReplayBuffer
...
def run(config):
    model_dir = Path('./maac_double/models') / config.model_name
    ...
    # create the model
    model = AttentionSACDouble.init_from_env(
        env,
        tau=config.tau,
        pi_lr=config.pi_lr,
        q_lr=config.q_lr,
    ...
    while ep_i < config.n_episodes:
        ...
        for et_i in range(config.episode_length):
            ...
            next_obs, rewards, dones, infos = env.step(
                np.array(actions[0] + actions[2]),
                np.array(actions[1] + actions[3]),
                order='team')
            next_obs =  parse_double(next_obs)
            replay_buffer.push(obs, agent_actions,
                               parse_double(rewards), next_obs,
                               parse_double(dones))
            obs = next_obs
