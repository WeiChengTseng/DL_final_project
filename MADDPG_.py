...
class Maddpg:
    ...
    def select_action_train(self,strikers,goalies):
        ...
        s_act_distr[even]=(self.s_actor[0](torch.from_numpy(strikers[even]).float().cuda())).clone()
        g_act_distr[even]=(self.g_actor[0](torch.from_numpy(goalies[even]).float().cuda())).clone()
        s_act_distr[odd]=(self.s_actor[1](torch.from_numpy(strikers[odd]).float().cuda())).clone()
        g_act_distr[odd]=(self.g_actor[1](torch.from_numpy(goalies[odd]).float().cuda())).clone()
    ...
    def select_action_test(self,strikers,goalies,team):
        ...
        if team == "red":
            ...
            s_action=self.s_actor[0](torch.from_numpy(strikers[odd]).float().cuda()).exp().detach()
            g_action=self.g_actor[0](torch.from_numpy(goalies[odd]).float().cuda())
    ...
    def update_policy(self, memory, step, writer,seed):
        ...
        for agent_index in range(self.n_striker):
            ...
            #     output =self.s_actor_other[agent_index][i](torch.from_numpy(np.stack(states[i+1],axis=0)).float().cuda()).clone()
            ...
            output_itself = self.s_actor_target[agent_index](torch.from_numpy(np.stack(next_states[0],axis=0)).float().cuda())
            if agent_index == 0:
                output_1 = self.s_actor_target[agent_index+1](torch.from_numpy(np.stack(next_states[1],axis=0)).float().cuda()).clone()
                output_2 = self.g_actor_target[agent_index](torch.from_numpy(np.stack(next_states[2],axis=0)).float().cuda()).clone()
                output_3 = self.g_actor_target[agent_index+1](torch.from_numpy(np.stack(next_states[3],axis=0)).float().cuda()).clone()
            else:
                output_1 = self.s_actor_target[agent_index-1](torch.from_numpy(np.stack(next_states[1],axis=0)).float().cuda()).clone()
                output_2 = self.g_actor_target[agent_index](torch.from_numpy(np.stack(next_states[2],axis=0)).float().cuda()).clone()
                output_3 = self.g_actor_target[agent_index-1](torch.from_numpy(np.stack(next_states[3],axis=0)).float().cuda()).clone()
            ...
            next_states_stack = torch.from_numpy(np.stack(next_states,axis=1)).float().cuda().reshape(self.batchSize, -1)
            target_Q_p=self.s_critic_target[agent_index](next_states_stack,critic_actions)
            states_stack = torch.from_numpy(np.stack(states,axis=0)).reshape(self.batchSize, -1).float().cuda().detach()
            probs_stack= torch.cat([torch.stack(probs[0]).detach(),torch.stack(probs[1]).detach(),torch.stack(probs[2]).detach(),torch.stack(probs[3]).detach()],dim=1).float().cuda().detach()
            if o_p == 0:
                print("probs_stack size: ",probs_stack.size())
            s_current_Q = self.s_critic[agent_index](states_stack,probs_stack)
            s_target_Q = target_Q_p.detach() * self.Gamma + torch.from_numpy(np.stack(rewards[0],axis=0) * self.scale_reward).unsqueeze_(-1).float().cuda()
            ...
            ###################################################
            # update actor policy
            output = self.s_actor[agent_index](torch.from_numpy(np.stack(states[0],axis=0)).float().cuda()).clone()
            probs_stack= torch.cat([output,torch.stack(probs[1]).detach(),torch.stack(probs[2]).detach(),torch.stack(probs[3]).detach()],dim=1).float().cuda()
        ...
        for agent_index in range(self.n_goalie):
            ...
            #     distr_stack= torch.stack(probs[i+1]).detach()
            #     output =self.g_actor_other[agent_index][i](torch.from_numpy(np.stack(states[i+1],axis=0)).float().cuda()).clone()
            #     dist_1 = Categorical(probs=output)
            #     dist_1_entropy = dist_1.entropy()
            ...
            output_itself = self.g_actor_target[agent_index](torch.from_numpy(np.stack(next_states[0],axis=0)).float().cuda())
            if agent_index == 0:
                output_1 = self.g_actor_target[agent_index+1](torch.from_numpy(np.stack(next_states[1],axis=0)).float().cuda()).clone()
                output_2 = self.s_actor_target[agent_index](torch.from_numpy(np.stack(next_states[2],axis=0)).float().cuda()).clone()
                output_3 = self.s_actor_target[agent_index+1](torch.from_numpy(np.stack(next_states[3],axis=0)).float().cuda()).clone()
            else:
                output_1 = self.g_actor_target[agent_index-1](torch.from_numpy(np.stack(next_states[1],axis=0)).float().cuda()).clone()
                output_2 = self.s_actor_target[agent_index](torch.from_numpy(np.stack(next_states[2],axis=0)).float().cuda()).clone()
                output_3 = self.s_actor_target[agent_index-1](torch.from_numpy(np.stack(next_states[3],axis=0)).float().cuda()).clone()
            
            critic_actions = torch.cat([output_itself.detach(),output_1.detach(),output_2.detach(),output_3.detach()],dim= 1).float().cuda().reshape(self.batchSize, -1)
            if o_p == 0:
                print(critic_actions.size())
            next_states_stack = torch.from_numpy(np.stack(next_states,axis=1)).float().cuda().reshape(self.batchSize, -1)
            target_Q_p=self.g_critic_target[agent_index](next_states_stack,critic_actions)
            states_stack = torch.from_numpy(np.stack(states,axis=0)).reshape(self.batchSize, -1).float().cuda()
            probs_stack= torch.cat([torch.stack(probs[0]).detach(),torch.stack(probs[1]).detach(),torch.stack(probs[2]).detach(),torch.stack(probs[3]).detach()],dim=1).reshape(self.batchSize, -1).float().cuda()
            g_current_Q = self.g_critic[agent_index](states_stack,probs_stack)
            g_target_Q = target_Q_p.detach() * self.Gamma + torch.from_numpy(np.stack(rewards[0],axis=0) * self.scale_reward).unsqueeze_(-1).float().cuda()
            ...
            output = self.g_actor[agent_index](torch.from_numpy(np.stack(states[0],axis=0)).float().cuda()).clone()
            probs_stack= torch.cat([output,torch.stack(probs[1]).detach(),torch.stack(probs[2]).detach(),torch.stack(probs[3]).detach()],dim=1).reshape(self.batchSize, -1).float().cuda()
