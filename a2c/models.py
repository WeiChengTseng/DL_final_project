...
def atari_initializer(module):
    ...
    Args:
    - module (nn.Module): the module that contains the parameters 
                          to be initialize.
    """
    classname = module.__class__.__name__

    if classname == 'Linear':
        module.weight.data = ortho_weights(module.weight.data.size(),
                                           scale=np.sqrt(2.))
...
class A2CLSTM(nn.Module):
    ...
    def forward(self, conv_in, h, c):
        ...
        lstm_out = self.lstm(shared_out, (h, c))
