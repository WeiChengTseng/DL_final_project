...
from maac_team.utils.buffer import ReplayBuffer
from maac_team.attention_sac import AttentionSACTeam
...
def run(config):
    model_dir = Path('./maac_team/models') / config.model_name
    ...
    # create the model
    model = AttentionSACTeam.init_from_env(
        env,
        tau=config.tau,
        pi_lr=config.pi_lr,
        q_lr=config.q_lr,
    ...
    while ep_i < config.n_episodes:
        ...
        for et_i in range(config.episode_length):
            ...
            next_obs, rewards, dones, infos = env.step(np.array(actions[0]),
                                                       np.array(actions[1]),
                                                       order='team')
